...
* Allt du behöver finns i den här filen – `README.md` – som ligger i
  projektmappen `Aktiescanner-`. Öppna den gärna i VS Code (vänster list i VS
  Code visar filen, dubbelklicka för att läsa).
* När jag uppdaterar guiden är det den här filen du ska läsa om igen. På så
  sätt behöver du aldrig leta på andra ställen eller gissa vad som gäller.
...
1. **Packa upp projektet**
   ...
   3. Flytta hela mappen `Aktiescanner-` till `Skrivbordet`. Nu finns filen
      `README.md` inuti den mappen.
...
3. **Gå in i mappen `Aktiescanner-`**
   1. Skriv exakt det här (utan citattecken) och tryck Enter:
      ...
      *Kommandot börjar med `cd` (för "change directory").
      `%USERPROFILE%` är en automatisk genväg till din användarmapp.

   2. Om du valde ett annat namn eller annan plats måste du justera sökvägen.
      Exempel för en mapp som heter `Aktie Program` i dokument:
...
7. **Hitta rapporten**
   ...
   1. Öppna Utforskaren.
   2. Gå till `Skrivbordet` → öppna mappen `Aktiescanner-` → öppna mappen
      `reports`.
...
> något i stil med `(.venv) C:\Users\DittNamn\Desktop\Aktiescanner->`. Om du
...
3. **Gå till projektmappen**. Om du sparade projektet i mappen `Aktiescanner-`
   ...
   > kommandot – alltså `cd Aktiescanner-` – och sedan trycka Enter. Orden som
   > står ovanför kodrutan (till exempel "bash" eller "text") är bara etiketter
   > i dokumentationen och ska inte skrivas i terminalen.
...
I mappen `scripts` finns filen `windows_quickstart.cmd`. Gör så här om du hellre
vill dubbelklicka på en fil:

1. Kopiera hela projektmappen `Aktiescanner-` till exempelvis Skrivbordet.
2. Dubbelklicka på `scripts\windows_quickstart.cmd`.
3. Följ instruktionerna i fönstret. Skriptet försöker först hitta mappen
   automatiskt. Om det inte lyckas får du skriva in sökvägen (klistra in från
   Utforskaren går bra).
4. När skriptet är klart ligger rapporten i
   `Aktiescanner-\reports\stockholm_large_midcap_metrics.csv`.
